...
class EditProductView extends StatelessWidget {
  ...
  EditProductView({Key? key, required this.product}) : super(key: key) {
    // Pre-fill form dengan data produk yang akan diedit
    _initializeFormData();
  }
  ...
  void _initializeFormData() {
    ...
    controller.isStockEnabled.value = product.isStockEnabled;

    // Reset selected image and show existing image if any
    controller.selectedImage.value = null;
    if (product.imageUrl != null) {
      // We'll handle existing image display separately
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          Obx(() => TextButton.icon(
            onPressed: controller.isLoading.value ? null : () => _updateProduct(),
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                if (product.imageUrl != null || controller.selectedImage.value != null)
                  TextButton.icon(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: Obx(() {
                      // Priority: newly selected image > existing image > placeholder
          ...
          children: [
            ...
            Container(
              ...
              child: TextField(
                ...
                enabled: false, // SKU tidak bisa diubah
                decoration: const InputDecoration(
                  border: InputBorder.none,
                  contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'Peringatan: Stok saat ini (${controller.stockValue.value}) <= stok minimum (${controller.minStockValue.value})',
                                style: TextStyle(
  ...
  Future<void> _updateProduct() async {
    try {
      if (!await controller.validateEditForm(product.id)) return;

      controller.isLoading.value = true;

      // Determine stock values based on tracking setting
      final int actualStock = controller.isStockEnabled.value
          ? int.parse(controller.stockController.text)
          : 999999; // Unlimited stock representation

      final int actualMinStock = controller.isStockEnabled.value
          ? int.parse(controller.minStockController.text)
          : 0;
      ...
      await controller.updateProduct(
        ...
        sku: controller.skuController.text.trim(),
        barcode: controller.barcodeController.text.trim(),
        costPrice: double.parse(controller.costPriceController.text),
        sellingPrice: double.parse(controller.sellingPriceController.text),
        unit: controller.unitController.text.trim(),
        stock: actualStock,
        minStock: actualMinStock,
        imageFile: controller.selectedImage.value,
        removeImage: controller.selectedImage.value == null && product.imageUrl != null,
        isStockEnabled: controller.isStockEnabled.value,
      );
    } catch (e) {
      Get.snackbar(
        'Error',
  ...
  void _removeImage() {
    ...
    Get.snackbar(
      'Berhasil',
      'Foto akan dihapus saat produk diupdate',
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                controller.resetForm(); // Reset form data
  ...
  bool _hasUnsavedChanges() {
    return controller.nameController.text != product.name ||
        controller.selectedCategoryId.value != product.categoryId ||
        controller.barcodeController.text != product.barcode ||
        controller.costPriceController.text != product.costPrice.toString() ||
        controller.sellingPriceController.text != product.sellingPrice.toString() ||
